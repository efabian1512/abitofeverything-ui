...
const Products = () => {
    ...
    return <div className="row">
        ...
        {filteredProducts?.map((product: any) => <div className={`col-md-4 col-sm-4 col-lg-4 mb-3`} key={product.id}>
            <ProductCard showActions={true} cardInfo={{ title: product.title, price: product.price, productImage: 'data:' + product.imageType+';base64,' + product.productImage, linkProperties: {desttination: '/shopping-cart', tag: 'Agregar al carrito' } }}/>
        </div>)}
        </div>
        </div>
    </div>
